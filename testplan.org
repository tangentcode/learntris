...
. . . .
= ) : clockwise rotation
: The ')' command rotates the actiev tetramino 90 degrees clockwise.
...
r . .
= Z Rotation
: The Z rotates within its 3x3 grid.
#+end_src
...
: The '>' command nudges the tetramino one cell the right.
#+end_src
...
: against the left side of the well. But it should go no further,
...
: on the right side of the well! Let's teach it a lesson, shall we?
#+end_src
...
: to plow its way through the right side of the well.
:
...
- [ ] A high score system.
- [ ] Rules for when a tetramino becomes part of the well.
